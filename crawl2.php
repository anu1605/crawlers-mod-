...
include "dependencies/crawl_functions2.php";
...
$epapers = array("NBT" => "Navbharat Times,hin", "NYB" => "Niyomiya Barta,asm", "PAP" => "Purvanchal Prahari,ori", "RS" => "Rashtriya Sahara,hin", "SAM" => "Sambad,ori", "SMJ" => "Samaja,ori", "SY" => "Samyukta Karnataka,kan", "VV" => "Vijayavani,kan", "YB" => "yashobhumi,hin", "SBP" => "Sangbad Pratidin,ben", "POD" => "Pratidin Odia Daily,ori", "MM" => "Mysore Mithra,kan", "MC" => "Mumbai Chaufer,mar", "NHT" => "Nav Hind Times,eng", "OHO" => "O Herald O,eng");
...
foreach ($epapers as $epapercode => $epaperArray) {
    ...
    if ($epapercode == "NHT") {
        $data = file_get_contents("https://epaper.navhindtimes.in/mainpage.aspx?pdate=" . $filenamedate . "");
        $contentArray = explode('id="DL_PageThumb_Img_Pagethumb_', $data);

        // if ($no_of_pages_to_run_on_each_edition > 0 and $no_of_pages_to_run_on_each_edition < count($pageArray)) $pageArray = array_slice($pageArray, 0, $no_of_pages_to_run_on_each_edition);
        for ($page = 1; $page <= count($contentArray); $page++) {
            $pagelink = str_replace(".jpg", ".pdf", str_replace("ss", "", str_replace("PageThumb", "pdf", explode('"', explode('src="', $contentArray[$page])[1])[0])));
            // $imagelink = explode('"', explode('id="artimg"  src="', $content)[1])[0];
            // $imageInfo = getimagesize($imagelink);
            // if (!$imageInfo)
            //     break;
            ...
            // writeImage($pagelink, $getpath[0]);

            echo date('Y-m-d H:i:s', time() + (5.5 * 3600)) . "=>File " . $getpath[0] . " Saved\n";
            runTesseract("Mumbai", $page, $section, "$conn", $getpath, $lang, $_GLOBAL['numbersArray']);
            echo date('Y-m-d H:i:s', time() + (5.5 * 3600)) . "=>" . " Page " . $page . " Section " . $section . " Completed\n";
            ob_flush();
            flush();
    ...
    //     for ($page = 1; $page <= $no_of_pages_to_run_on_each_edition; $page++) {
    //         $testcontent = file_get_contents("http://epaper.heraldgoa.in/articlepage.php?articleid=OHERALDO_GOA_" . $dateForLinks . "_" . $page . "_1");
    ...
    //         for ($section = 1; $section < $no_of_sections_to_run_on_each_page; $section++) {
    //             $content = file_get_contents("http://epaper.heraldgoa.in/articlepage.php?articleid=OHERALDO_GOA_" . $dateForLinks . "_" . $page . "_" . $section);
